...
class MyAI extends StatefulWidget {
  @override
  _MyAIState createState() => _MyAIState();
}
...
class _MyAIState extends State<MyAI> with TickerProviderStateMixin {
  ...
  Future<void> _saveAnalysis(Map<String, dynamic> analysis) async {
    ...
    final analysisData = {
      ...
      'mostProbableDisease': analysis['mostProbableDisease'],
  ...
  Widget _buildAnalysisResult(Map<String, dynamic> result) {
    ...
    final mostProbableDisease = result['mostProbableDisease'];
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Disease Information
          if (!isHealthy && mostProbableDisease != null) ...[
            _buildDiseaseCard(mostProbableDisease),
            const SizedBox(height: 20),
          ],
          ...
          if (suggestions.isNotEmpty) ...[
            ...
            const SizedBox(height: 12),
            ...suggestions.map((suggestion) => _buildSuggestionItem(suggestion.toString())),
          ] else if (!isHealthy) ...[
            // Show generic care tips if no specific suggestions
            Text(
              "General Care Tips:",
              style: GoogleFonts.poppins(
                fontSize: 16,
            ...
            const SizedBox(height: 12),
            _buildSuggestionItem("Monitor the plant closely for changes"),
            _buildSuggestionItem("Ensure proper watering and drainage"),
            _buildSuggestionItem("Remove affected leaves if necessary"),
            _buildSuggestionItem("Consult a plant expert for specific treatment"),
          ],
  ...
  Widget _buildDiseaseCard(Map<String, dynamic> disease) {
    final name = disease['name']?.toString() ?? 'Unknown Disease';
    final probability = disease['probability'] ?? 0.0;
    final treatment = disease['treatment']?.toString() ?? 'No treatment info available.';
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Icon(Icons.analytics, color: Colors.red.shade600, size: 18),
  ...
  Widget _buildRecentAnalysisItem(Map<String, dynamic> analysis) {
    ...
    final disease = analysis['mostProbableDisease'];
              ...
              children: [
                Text(
                  isHealthy
                      ? "Healthy Plant"
                      : disease != null
                      ? disease['name']?.toString() ?? 'Disease Detected'
                      : 'Disease Detected',
                  style: GoogleFonts.poppins(
                    fontSize: 14,
                    fontWeight: FontWeight.w500,
